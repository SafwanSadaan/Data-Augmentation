...
def choose_quality():
    quality = simpledialog.askinteger("إدخال الجودة", "اختر جودة الصورة أو الفيديو (من 1 إلى 100):", minvalue=1, maxvalue=100)
    if quality is not None:
        return quality
    return 80  # القيمة الافتراضية إذا لم يحدد المستخدم جودة

# معالجة الصور باستخدام تقنيات تعزيز البيانات المتقدمة
def process_images(source_file, output_dir, num_files, image_quality=80):
    image = Image.open(source_file)
    # تقليل دقة الصورة (تغيير الحجم)
    base_width = 800  # تحديد عرض ثابت
    w_percent = base_width / float(image.size[0])
    h_size = int((float(image.size[1]) * float(w_percent)))
    image = image.resize((base_width, h_size), Image.ANTIALIAS)  # تقليل حجم الصورة

    for i in range(1, num_files + 1):
        augmented_image = augment_image(image)
        output_file = os.path.join(output_dir, f"{i}{os.path.splitext(source_file)[1]}")
        # تقليل الجودة أثناء الحفظ (لحجم أقل)
        augmented_image.save(output_file, quality=image_quality)  # الجودة تكون من اختيار المستخدم
...
def augment_image(image):
    ...
    # 3. إضافة الضبابية (يمكن تقليل الحجم قليلاً عن طريق تقليل الضبابية)
    if random.choice([True, False]):
        image = image.filter(ImageFilter.GaussianBlur(random.uniform(0.5, 1)))  # تقليل الضبابية
    ...
    if random.choice([True, False]):
        ...
        noise = np.random.normal(0, 15, np_image.shape)  # تقليل الضوضاء لتقليل الحجم
...
def process_videos(source_file, output_dir, num_files, compression='mp4v'):
    ...
    for i in range(1, num_files + 1):
        ...
        if not out.isOpened():
            messagebox.showerror("خطأ", f"تعذر إنشاء ملف الفيديو: {output_file}")
            continue
        ...
        noise_strength = random.uniform(0, 25)
        ...
        selected_effects = [
            ...
            lambda frame: apply_noise(frame, noise_strength)
        ]
        ...
        while True:
            ...
            # كتابة الإطار المعدل إلى الفيديو الجديد
            out.write(frame)
...
# زر التحويل
tk.Button(fram, text="بدء العملية", command=process_files, font=("Helvetica", 14), width=25, height=1, bg="#2ecc71", fg="white").grid(row=6, column=0, padx=10, pady=20)
